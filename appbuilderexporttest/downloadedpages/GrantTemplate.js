...
function unwrap_dynamically(value,default_value){
 ...
 class GrantTemplate extends React.Component {
    ...
    render(){ 
      ...
      <Text
          style= {[{position:'absolute',zIndex:100,width:'100%'},{"innerText":"  JSON.parse(JSON.stringify(data[appData.firstGrant][\"Who the grant is for\"]));","top":"14.43%","left":"5.31%","height":"1%","width":"80%"}]}
        > {  JSON.parse(JSON.stringify(data[appData.firstGrant]["Who the grant is for"]))} </Text>
        
<Text
          style= {[{position:'absolute',zIndex:100,width:'100%'},{"innerText":"JSON.parse(JSON.stringify(data[appData.firstGrant][\"Deadline to apply for grant\"]))","top":"24.04%","left":"6.03%"}]}
...
<Text
          style= {[{position:'absolute',zIndex:100,width:'100%'},{"innerText":"JSON.parse(JSON.stringify(data[appData.firstGrant][\"Grant focus areas (location)\"]))","top":"64.2%","left":"6.03%"}]}
        > {JSON.parse(JSON.stringify(data[appData.firstGrant]["Grant focus areas (location)"]))} </Text>
        
<Text
          style= {[{position:'absolute',zIndex:100,width:'100%'},{"innerText":"JSON.parse(JSON.stringify(data[appData.firstGrant][\"Amount provided by grants\"]))","top":"75.21%","left":"6.51%"}]}
        > {JSON.parse(JSON.stringify(data[appData.firstGrant]["Amount provided by grants"]))} </Text>
        
<Text
          style= {[{position:'absolute',zIndex:100,width:'100%'},{"innerText":"'Recipient Type:'","top":"11.63%","left":"5.55%","color":"orange","fontWeight":"bold","height":"1%","textAlign":"left"}]}
        > {'Recipient Type:'} </Text>
...
<Text
          style= {[{position:'absolute',zIndex:100,width:'100%'},{"innerText":"'Geograhic Scope:'","top":"61.88%","left":"5.55%","height":25,"color":"orange","fontWeight":"bold"}]}
        > {'Geograhic Scope:'} </Text>
        
<Text
          style= {[{position:'absolute',zIndex:100,width:'100%'},{"innerText":"'Grant amount:'","top":"72.73%","left":"6.27%","color":"orange","fontWeight":"bold"}]}
        > {'Grant amount:'} </Text>
...
<View
 ...
 <TouchableOpacity
          ...
          style= {[{
            ...
            height:"7%",
            width:"30%",
            position:'absolute',top:0,left:0, backgroundColor:'#8fd158', alignItems:'center',justifyContent:'center', height: "7%",  title:'Test', borderColor: 'gray', color:'black', borderRadius:15, borderWidth: 1},{"innerText":"'Return to Search'","top":"86.69%","left":"36.21%"}]}
        >
...
<Text
          style= {[{position:'absolute',zIndex:100,width:'100%'},{"textAlign":"center","innerText":" JSON.parse(JSON.stringify(data[appData.firstGrant][\"Grantor Name\"]));","color":"orange","fontWeight":"bold","top":"1.71%","left":"11.06%","width":"80%"}]}
        > { JSON.parse(JSON.stringify(data[appData.firstGrant]["Grantor Name"]))} </Text>
        
 <TouchableOpacity
          
          onPress = { function(){that.props.saveTo("userdata",{"Username": appData.LoginScreeninput2, "Grant Indexes": appData.firstGrant});; that.forceUpdate(); }}  
          style= {[{
            shadowColor: 'rgba(0,0,0, .4)', // IOS
            shadowOffset: { height: 1, width: 1 }, // IOS
            shadowOpacity: 1, // IOS
            ...
            height:"7%",
            width:"30%",
            position:'absolute',top:0,left:0, backgroundColor:'#8fd158', alignItems:'center',justifyContent:'center', height: "7%",  title:'Test', borderColor: 'gray', color:'black', borderRadius:15, borderWidth: 1},{"top":"68.55%","left":"61.12%","innerText":"'Save Grant'"}]}
        >
        <Text style = {{color:"black"}}>
